"""
시드 데이터 삽입 스크립트.
배포 환경에서 데모 데이터를 생성한다.

실행 방법:
  로컬:   cd backend && python test_seed.py
  Docker: docker compose exec app python test_seed.py
  초기화: python test_seed.py --reset  (기존 데이터 삭제 후 재생성)
"""
...
def has_existing_data(db) -> bool:
    """시드 데이터가 이미 존재하는지 확인한다."""
    group_count = db.query(Group).count()
    user_count = db.query(User).count()
    return group_count > 0 and user_count > 0
...
def seed(db):
    ...
    # 1. 관리자
    # ════════════════════════════════════════════
    admin1 = db.query(Admin).filter(Admin.emp_no == DEFAULT_EMP_NO).first()
    if not admin1:
        admin1 = Admin(emp_no=DEFAULT_EMP_NO)
        db.add(admin1)
        db.flush()

    admin2 = db.query(Admin).filter(Admin.emp_no == "20260401").first()
    if not admin2:
        admin2 = Admin(emp_no="20260401")
        db.add(admin2)
        db.flush()
    print(f"관리자: {admin1.emp_no} (id={admin1.id}), {admin2.emp_no} (id={admin2.id})")
    ...
    groups = []
    for admin_id, name in group_defs:
        g = db.query(Group).filter(Group.name == name).first()
        if not g:
            g = Group(admin_id=admin_id, name=name)
            db.add(g)
            db.flush()
        groups.append(g)
    ...
    for i, lang in enumerate(languages):
        ...
        existing = db.query(User).filter(User.system_id == sys_id).first()
        if existing:
    ...
    # --- 이번 달 과거 데이터 ---
    month_sessions = 0
    for day_offset in range(2, min(TODAY.day, 13)):
...
def main():
    parser = argparse.ArgumentParser(description="AISG 시드 데이터 삽입")
    parser.add_argument("--reset", action="store_true", help="기존 데이터를 삭제하고 재생성")
    ...
    db = SessionLocal()
    try:
        if not args.reset and has_existing_data(db):
            print("시드 데이터가 이미 존재합니다. 초기화하려면 --reset 플래그를 사용하세요.")
            sys.exit(0)
